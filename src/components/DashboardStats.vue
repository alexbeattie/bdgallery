<template>
  <div class="bg-white dark:bg-mcm-gray-800 rounded-lg shadow-md overflow-hidden">
    <div class="px-4 py-5 sm:p-6">
      <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Event Statistics</h3>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="bg-mcm-amber-50 dark:bg-mcm-amber-900/20 rounded-lg p-4">
          <div class="flex items-center">
            ...
            <div>
              <div class="text-xs text-mcm-amber-600 dark:text-mcm-amber-300 uppercase font-medium tracking-wide">
                Total RSVPs
              </div>
          ...
          <div class="mt-2 flex justify-between items-center text-xs text-mcm-amber-600 dark:text-mcm-amber-300">
            <div>
        ...
        <div class="bg-mcm-teal-50 dark:bg-mcm-teal-900/20 rounded-lg p-4">
          <div class="flex items-center">
            <div class="h-10 w-10 rounded-full bg-mcm-teal-100 dark:bg-mcm-teal-900 flex items-center justify-center text-mcm-teal-600 dark:text-mcm-teal-300 mr-3 flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <div>
              <div class="text-xs text-mcm-teal-600 dark:text-mcm-teal-300 uppercase font-medium tracking-wide">
                Photos Shared
              </div>
          ...
          <div class="mt-2 flex justify-between items-center text-xs text-mcm-teal-600 dark:text-mcm-teal-300">
            <div>
        ...
        <div class="bg-mcm-mint-50 dark:bg-mcm-mint-900/20 rounded-lg p-4">
          <div class="flex items-center">
            <div class="h-10 w-10 rounded-full bg-mcm-mint-100 dark:bg-mcm-mint-900 flex items-center justify-center text-mcm-mint-600 dark:text-mcm-mint-300 mr-3 flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <div>
              <div class="text-xs text-mcm-mint-600 dark:text-mcm-mint-300 uppercase font-medium tracking-wide">
                Days Until Party
              </div>
        ...
        <div class="bg-mcm-mustard-50 dark:bg-mcm-mustard-900/20 rounded-lg p-4">
          <div class="flex items-center">
            <div class="h-10 w-10 rounded-full bg-mcm-mustard-100 dark:bg-mcm-mustard-900 flex items-center justify-center text-mcm-mustard-600 dark:text-mcm-mustard-300 mr-3 flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
            </div>
            <div>
              <div class="text-xs text-mcm-mustard-600 dark:text-mcm-mustard-300 uppercase font-medium tracking-wide">
                New Messages
              </div>
              <div class="text-2xl font-bold text-mcm-mustard-700 dark:text-mcm-mustard-200">
                {{ stats.totalMessages }}
              </div>
            </div>
          </div>
          <div class="mt-2 text-xs text-mcm-mustard-600 dark:text-mcm-mustard-300">
            <div>
              Last message from <span class="font-medium">{{ stats.lastMessageFrom }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-4">
          <div v-for="(activity, index) in recentActivity" :key="index" class="flex">
            <div class="flex-shrink-0 mr-3">
              <div class="h-8 w-8 rounded-full bg-gray-100 dark:bg-mcm-gray-700 flex items-center justify-center">
          ...
          <div v-if="recentActivity.length === 0" class="text-center text-sm text-gray-500 dark:text-gray-400 py-4">
            No recent activity
          </div>
        </div>
      </div>
  ...
  setup(props) {
    ...
    const fetchStats = async () => {
      try {
        ...
        userSnapshot.forEach(doc => {
          ...
          if (userData.rsvp === 'yes') {
            confirmedAttending++;
          } else if (userData.rsvp === 'no') {
            declined++;
          }
        });
        
        // Fetch photo stats
        const photosQuery = query(collection(db, 'birthdayImages'));
        ...
        photoSnapshot.forEach(doc => {
          ...
          if (photoData.userId) {
            contributors.add(photoData.userId);
          }
          
          if (photoData.message && photoData.message.trim() !== '') {
            photosWithMessages++;
            
            if (photoData.timestamp && (!lastMessageTime || photoData.timestamp.toDate() > lastMessageTime)) {
        ...
        // Update stats
        stats.value = {
          totalRsvps: confirmedAttending + declined,
          confirmedAttending,
          declined,
          totalPhotos,
          ...
          daysUntilParty: props.daysUntilParty,
          partyDate: 'April 26th, 2025',
          totalMessages: photosWithMessages,
          lastMessageFrom
        };
    ...
    const fetchRecentActivity = async () => {
      try {
        ...
        const photoSnapshot = await getDocs(recentPhotosQuery);
        
        const photosActivity = photoSnapshot.docs.map(doc => {
          const data = doc.data();
          return {
            type: 'photo',
